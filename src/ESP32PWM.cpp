...
double ESP32PWM::setup(uint8_t pin, double freq, uint8_t resolution_bits) {
    checkFrequencyForSideEffects(freq);

    resolutionBits = resolution_bits;
    if (attached()) {
#if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 0, 0)
        ledcDetach(getPin()); // Detach old pin
        double val = ledcAttach(pin, freq, resolution_bits); // Attach new pin
#else
        ledcDetachPin(getPin()); // detach old pin
...
//					Serial.println("Timer out of channels target="+String(freq)+" on timer "+String(i)+" was "+String(timerCount[i]));
    ...
    ESP_LOGE(TAG, "ERROR All PWM timers allocated! Can't accomodate %d Hz\r\nHalting...", freq);
    while (1)
        ;
}
void ESP32PWM::deallocate() {
